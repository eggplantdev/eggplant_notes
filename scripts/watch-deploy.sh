...
# Resolves projectId/orgId from .vercel/project.json (single-project link) OR
# .vercel/repo.json (repo-level link, `vercel link --repo`) — project-agnostic,
# drop into either link shape unchanged.
set -euo pipefail
cd "$(dirname "$0")/.."

# Two link shapes carry the IDs differently:
#   .vercel/project.json → { projectId, orgId }                (vercel link)
#   .vercel/repo.json    → { projects: [{ id, orgId }, ...] }  (vercel link --repo)
# Prefer project.json when present; otherwise read the first project in repo.json.
if [ -f .vercel/project.json ]; then
  ...
  TEAM_ID="$(jq -r '.projects[0].orgId' .vercel/repo.json)"
else
  echo "watch-deploy: no .vercel/project.json or .vercel/repo.json — run 'vercel link' first" >&2
  exit 1
fi
