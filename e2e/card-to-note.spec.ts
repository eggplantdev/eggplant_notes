...
// S-08: from a due recall card on the dashboard review panel, the user jumps to the card's
// source note in one action. Sign up → create a note (capture its URL) → attach a check (due_at
// defaults to now → immediately due) → open /dashboard → assert the card header links to the
