...
-- — the existing per-action topic_checks_*_own policies already gate the whole row, and the
-- SM-2 write path remains S-03's alone.

alter table topic_checks add column example text;
alter table topic_checks add column code_context text;
