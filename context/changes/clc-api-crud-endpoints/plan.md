...
- [ ] 2.3 Unit tests pass (incl. unchanged action tests): `pnpm test`
- [ ] 2.4 Integration tests pass (move-all + forced-unlink): `pnpm test:integration`
- [ ] 2.5 Skill-template pinning test passes: `pnpm test src/__tests__/skill-template.test.ts`
- [ ] 2.6 Production build succeeds: `pnpm build`

#### Manual

- [ ] 2.7 PATCH note subject (no card_actions) → linked cards moved, still linked
- [ ] 2.8 PATCH note with card_actions.unlink → those cards note_id null, old subject kept
- [ ] 2.9 PATCH attached card's subject → unlinked (note_id null); field-only edit keeps link
- [ ] 2.10 UI note/card edit + note-move dialog behave identically post-refactor
