...
| 7   | **Untrusted markdown runs script in the user's session** — an imported `.md` or an AI-generated note body renders raw HTML (`<script>`, `on*=`) or a `javascript:`/`data:` URL that executes in the authenticated session (JWT/session theft, acting as the user). Protection rests entirely on react-markdown defaults (no `rehype-raw`, default `urlTransform`) and is untested | High   | Med        | operator's explicit worry (malicious scripts via markdown inputs); `context/changes/import-markdown-to-notes/` (first untrusted-source surface, in flight); R3 AI bodies (second untrusted source) |
...
| 7   | Markdown render XSS guard          | Prove untrusted markdown (imported file, AI body) renders inert — no script execution, dangerous URLs neutralized | #7 (complements #3)            | e2e (paste & observe)                                            | not started | —                                                              |
