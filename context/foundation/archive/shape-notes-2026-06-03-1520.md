...
v1 explicitly does NOT include the following. These are _scope_ avoids that block sneaking-back-in; technology avoids (specific frameworks, deployment platforms) belong with `/10x-tech-stack-selector` and are captured in the Forward: tech-stack block.
