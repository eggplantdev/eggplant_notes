...
| S-12 | seed-sample-data               | one-click load (and one-click clear) a starter subject + notes + cards to demo the app                                     | S-01, S-02, S-06 | US-01 (first-run); course-eval demo affordance | fast-follow | proposed |
...
### S-12: seed sample data (course-eval demo affordance)
...
- **Outcome:** a signed-in user on an **empty** account (no subjects, no notes, no topic checks — trivially detectable) sees a **"Load sample data"** button (empty-state on `/dashboard` / `/notes`, and in `/settings`). Clicking it populates their account with a starter **subject** + a few **notes** with highlighted code + a handful of **topic checks** (some made due, so the recall loop has cards to review and the dashboard shows non-zero state) — enough to showcase every feature. A paired **"Clear sample data"** button removes what was seeded, returning the account to empty.
...
- **Prerequisites:** S-01 (notes write path), S-02 (topic-check write path), S-06 (subjects write path)
- **Parallel with:** any v2 item; touches only the write paths these slices already shipped
- **Not the dev seed:** distinct from the **dev-only** `supabase/seed.sql` + `generate-section-seed.mjs` (those run on `supabase db reset`, off-Vercel, pre-made fixed accounts). This is an **in-app, runtime, per-user, RLS-scoped** seed triggered by the signed-in user — it inserts through the same Server Actions / RPCs (never `user_id` mass-assignment), so rows land under `auth.uid()` like any other write. The content is a **small, hand-curated, limited** set — NOT the full `/workspace/learning`-derived corpus the dev script generates.
- **Two buttons:**
  - **Load** — visible only when the account has **no non-seeded rows** (gate on a count of the user's subjects/notes); writes the curated fixture.
  - **Clear** — **delete-only-seeded (RESOLVED 2026-06-04):** removes _only_ the rows S-12 created, never the evaluator's own data. Narrower than S-05 (which nukes the whole account). Requires a marker so seeded rows are unambiguously identifiable.
- **Marker mechanism (leaning, `/10x-plan` to confirm):** a boolean `is_seeded` column on `subjects` / `notes` / `topic_checks` (default `false`), set `true` on the seed write. Cheap additive migration; makes both buttons trivial — Load gates on "no rows where `is_seeded = false`", Clear deletes `where is_seeded = true`. The fragile alternative (match a known subject name) is rejected — breaks if the evaluator renames it.
- **Unknowns:**
  - **Empty-detection + Load gating** — confirmed shape: "no non-seeded rows." `/10x-plan` to finalize where the button shows (dashboard/notes empty-state + settings). Block: no.
  - ~~**Clear scope / how to identify seeded rows.**~~ **Resolved 2026-06-04: delete-only-seeded, via an `is_seeded` marker column.**
  - **Atomicity** — reuse the existing `create_note_with_checks` RPC per note (subject → notes+checks) vs. a single new seed RPC. Owner: `/10x-plan`. Block: no.
- **Schema note:** the `is_seeded` marker is a small additive migration (defaulted boolean on three tables) — so S-12 is **no longer schema-free**. Still low risk; RLS policies are unchanged (the marker is just another owned column).
- **Risk:** Low — additive, one small marker migration, writes through existing paths. Fast-follow: directly serves the grading/demo need, not required to cross the deadline line — but high leverage for evaluation.
- **Status:** proposed
...
| S-12       | seed-sample-data               | Load + Clear sample data (course-eval demo)           | not yet               | Fast-follow. **For tutor grading** — one-click full-app demo, one-click clear. In-app, per-user, RLS-scoped runtime seed (NOT the dev `seed.sql`); curated/limited content. Writes through existing Server Actions / `create_note_with_checks` RPC. **Clear = delete-only-seeded via an `is_seeded` marker column** (small additive migration). Open: button placement, atomicity. Ready once `/10x-plan` resolves those |
