...
| S-17 | topic-checks-listing           | browse all recall cards on a `/topic-checks` page, filtered by subject server-side; jump from a card to its source note    | S-02, S-06, S-08                   | US-01, Scope:[new] card→note (listing)         | v2          | implemented\*\* |
...
>
> \*\* **S-17 `implemented**`** — ad-hoc slice (has a `/10x-new`change folder + plan; EX-385). Feature-complete, manually verified, and committed to`main` (`b08e0fe`→`a6e8009`). **Not yet archived**: the slice review gate (`/10x-impl-review`→`/simplify`→ tests) still has to run before`/10x-archive`, which is when this flips to `done`, the `## Done` record lands, and EX-385 flips to Done.
