...
| S-20 | expose-cli-note-api            | add a note (+ its cards) over HTTP from a CLI/agent skill, authed by a per-device personal API token (mint/revoke/audit)          | S-01, S-02, S-07                   | US-01 (authoring); net-new external surface    | v2 (last)      | done     |
...
- **Status:** done (archived 2026-06-07 → `context/archive/2026-06-06-import-markdown-to-notes/`)
...
- **(ad-hoc bugfix, not a PRD slice) Linked memory-cards always share their note's subject — closed two invariant holes** — `create_note_with_checks` (shared by the web create-note form AND `POST /api/notes`) inserted inline `checks`-cards with no `subject_id`, so a note created under a subject got linked-but-UNFILED cards — a silent break in BOTH UI and API (an integration test had even codified the null). Fixed the RPC to stamp the note's subject (migration `20260609140000`), matching `insertCardsForNote`. Also closed an API-only hole: `PATCH /api/notes` `card_actions` collapsed to `{unlink}`-only (core now unlinks **before** moving) so a partial plan can't strand a linked card on its old subject — the API can no longer reach a state the UI can't; the `clc-note-api` skill now also instructs the agent to tell the user which cards moved/detached. Reviewed live; suite green (typecheck/lint/236 unit/24 integration/build), e2e N/A (backend). Lesson: a cross-entity invariant must be enforced on every creation path. Linear: pending MCP re-auth.
- **S-20: add a note (+ its cards) over HTTP from a CLI/agent skill, authed by a per-device personal API token (mint/revoke/audit)** — Archived 2026-06-09 → `context/archive/2026-06-06-expose-cli-note-api/`. Keep-RLS token API, no service-role: `resolve_api_token` (DEFINER, anon) → minted short-lived JWT (`jose` HS256) → `accessToken` supabase-js client → existing INVOKER cores; 4 routes (POST/GET `/api/notes`, POST `/api/memory-cards`, GET `/api/subjects`); writes refactored to shared injectable `insert-*` cores (UI path unchanged). Phase-2 parked: device-flow token issuance, token UI, rate limiting, scope enforcement. Lesson: a `z.union` body whose members aren't cleanly distinguishable by Zod silently misroutes on fall-through — select the branch by raw discriminant presence before validating (caught + fixed at the review gate, F1).
