...
- **S-14: in-place editing for notes and subjects — light read-only by default; an `?edit` toggle swaps the detail page to the form (body+subject for notes; title/description header for subjects) without navigating away; the separate `/notes/[id]/edit` and `/subjects/[id]/edit` routes are removed.** — Archived 2026-06-04 → `context/archive/2026-06-04-inline-edit-notes-and-subjects/`. Built in worktree off `eb394bf` (commits `17f6ef8`→`aec7c33`→`4a0bd45`→`5f0428c`); review fan-out all APPROVED/PASS, `/simplify` no-op. E2E specs authored + verified green locally but suite-wide run is blocked by pre-existing S-16 toast-assertion debt in the shared `signUp` helper (deferred → `follow-ups/review-fixes.md`). Lesson: —.
