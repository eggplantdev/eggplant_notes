...
- **Parked — NOT deadline work.** Extra surface to pick up only after the basic version ships. Do not start ahead of the in-flight `list-search-pagination` and `seed-sample-data` work.
...
- **Status:** proposed (parked; `change.md` written, no `plan.md` yet → `context/changes/import-markdown-to-notes/`)
...
| S-09       | authoring-refinements          | Defer validation error + code-language select         | done\*                 | Fast-follow. **Done but not archived** (ad-hoc, no `/10x-new` folder, tests skipped). On `main`; EX-371 Done. Code-language inserter later reused in memory-card editors (`cd4634d`)                                                                                                                                                                                                                                                                                                                      |
...
| S-19       | import-markdown-to-notes       | Import a markdown file into notes (+ optional AI gen) | not yet                | v2 (**post-ship — parked**). Bundled + phased: **Phase 1** deterministic heading-split + preview/edit (no AI); **Phase 2** OpenRouter connect (**BYOK via OAuth PKCE**, encrypted per-user key) + AI note/card gen, **opt-in, gated to connected users**. Vercel Gateway suggestion scrapped. Existing `generate-section-seed.mjs` does NOT generalize. Design + open forks in `context/changes/import-markdown-to-notes/change.md`; needs `/10x-plan` (NOT `/10x-shape`) when unparked. Added 2026-06-06 |
