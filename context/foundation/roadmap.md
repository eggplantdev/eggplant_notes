...
- **Two buttons:**
  - **Load** — visible only when the account is empty (gate on a count of the user's subjects/notes); writes the curated fixture.
  - **Clear** — removes the seeded rows. Open: scope it to _only_ seeded rows (tag/marker) vs. "delete all my data" (the S-05 delete-account path already nukes everything; this should be narrower — clearing demo data, not the account).
- **Unknowns:**
  - **Empty-detection + Load gating** — what exactly counts as "empty" (zero notes? zero subjects AND zero notes?) and where the button shows. Owner: `/10x-plan`. Block: no.
  - **Clear scope / how to identify seeded rows** — mark seeded rows (e.g. a flag or a known subject name) so Clear deletes only those, vs. clearing all content. Owner: `/10x-plan`. Block: no.
...
- **Risk:** Low — additive, no schema change required if Clear keys off content rather than a new column (TBD by plan), writes through existing paths. Fast-follow: directly serves the grading/demo need, not required to cross the deadline line — but high leverage for evaluation.
- **Status:** proposed
