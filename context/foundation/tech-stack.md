...
- **Recall-scheduling algorithm: ts-fsrs (FSRS).** FR-018 calls for "an established modern adaptive algorithm." Resolved 2026-06-03 in S-03 `close-recall-loop`: **ts-fsrs** (FSRS-4.5/5, Anki's current default) over SuperMemo SM-2. Shipped in migration `20260603131542_fsrs_review_loop.sql` — `review_events.rating` is FSRS 1–4 (Again/Hard/Good/Easy), reversing F-02's interim SM-2 0–5 lock.
...
- **PWA: installable, no service worker.** Added 2026-06-21. Web manifest + home-screen icons via Next App Router metadata conventions (`src/app/manifest.ts`, `app/apple-icon.tsx`, generated `app/app-icon-{192,512}.png/route.tsx` through `ImageResponse`, reusing `brand-mark-dots`). `display: standalone`, `start_url: /dashboard`; the cookie session carries so installs launch already signed-in. Deliberately **no** service worker → no offline, always-live (a new deploy is picked up on the next load). The proxy must keep the manifest/icon routes public — `PUBLIC_ASSET_ROUTES` in `src/proxy.ts`, guarded by `src/__tests__/proxy-api-gate.test.ts`.
- **Forms + validation: TanStack Form + Zod.** Added in F-01 `minimal-auth-and-session` (2026-06-02). `@tanstack/react-form@^1.27.7` drives form state via a bound `useAppForm` (`createFormHook`/`createFormHookContexts`), mirrored from the `wykonczymy` reference repo. `zod@^4.3.5` (also mirrored from `wykonczymy`) is the single schema-validation library — schemas live with their feature (e.g. `src/features/auth/schema.ts`) and are reused for both client-side field validators (TanStack `validators`) and server-side Server Action input parsing (the generic `validateInput` helper, co-located in `src/features/auth/validate.ts` until a 2nd feature needs it). This matches the global React rule (TanStack Form for state, Zod for validation) and the feature-first layout in AGENTS.md.
