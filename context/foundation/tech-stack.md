...
- **Forms + validation: TanStack Form + Zod.** Added in F-01 `minimal-auth-and-session` (2026-06-02). `@tanstack/react-form@^1.27.7` drives form state via a bound `useAppForm` (`createFormHook`/`createFormHookContexts`), mirrored from the `wykonczymy` reference repo. `zod@^4.3.5` (also mirrored from `wykonczymy`) is the single schema-validation library — schemas live in `src/lib/.../*-schemas.ts` and are reused for both client-side field validators (TanStack `validators`) and server-side Server Action input parsing (`validateInput` helper in `src/lib/actions/validate.ts`). This matches the global React rule (TanStack Form for state, Zod for validation).
